...
from .models import LoginAttempt, Product, Traveler
...
def homepage(request):
    ...
    return render(request, 'homepage.html', {'login_attempts': login_attempts})
...
def login2(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #print (email)
            #print (password)
            login_attempt = LoginAttempt(email=email, password=input_password)
            ...
            if len(alltravelers) > 0:
                ...
                login_attempt.result = 'Failure'
                context = {
                    'message': 'Error, Password or Email are incorrect',
                }
                login_attempt.save()  # Save the login attempt to the database
                response_data = login_attempt.as_dict()
